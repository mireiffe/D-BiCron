...
핵심 모듈:
  ...
  - watermark : watermark 타입(serial/numeric/timestamp) 파싱 + since/overlap/retention 계산
  - connections: 접속 정보 레지스트리(JSON) + PG/CH/meta 커넥션 팩토리
  - config    : 테이블 파이프라인 + retention 설정(YAML) 로드 / 검증
  - tracking  : copy_run / copy_batch / copy_failed_row 메타 추적 저장소
  - copier    : append / full_reload 복사 오케스트레이션 (batch + row 단위 dead-letter)
  - retention : PG source row 배치 삭제 (타입별 cutoff, finalize 된 watermark 로 캡핑)
